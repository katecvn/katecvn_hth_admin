      ...
      <TooltipProvider delayDuration={0}>
        <nav className="grid gap-1 group-[[data-collapsed=true]]:justify-center group-[[data-collapsed=true]]:px-2">
          {links.map((link, index) => (
            // <Can permission={true} key={link.href}>
            <Can permission={link.permission} key={link.href + '-' + index}>
              {renderLink(link)}
            </Can>
          ))}
        </nav>
    ...
    <Collapsible defaultOpen={isChildActive}>
      ...
      <CollapsibleContent className="collapsibleDropdown" asChild>
        <ul>
          {sub &&
            sub.map((subLink) => (
              <Can permission={subLink.permission} key={subLink.href}>
      ...
      <DropdownMenuContent side="right" align="start" sideOffset={4}>
        ...
        {sub &&
          sub.map(({ title, icon, label, href, permission }) => (
            <Can permission={permission} key={href}>

...
const UserNav = () => {
  ...
  const fullName = authUserWithRoleHasPermissions?.full_name || ''
        ...
        <DropdownMenuTrigger asChild>
          <Button variant="ghost" className="relative h-8 w-8 rounded-full">
            <Avatar className="h-8 w-8">
              <AvatarImage
                src={`https://ui-avatars.com/api/?bold=true&background=random&name=${authUserWithRoleHasPermissions?.full_name}`}
                alt={authUserWithRoleHasPermissions?.full_name}
              />
          ...
          <DropdownMenuLabel className="font-normal">
            <div className="flex flex-col space-y-1">
              <p className="text-sm font-medium leading-none">
                {authUserWithRoleHasPermissions?.full_name}
              </p>
              <p className="text-xs leading-none text-muted-foreground">
                {authUserWithRoleHasPermissions?.roles &&
                  authUserWithRoleHasPermissions?.roles[0]?.name}
              </p>
            </div>
          </DropdownMenuLabel>

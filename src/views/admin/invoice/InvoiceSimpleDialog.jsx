...
const InvoiceSimpleDialog = ({ open, onOpenChange, order }) => {
        ...
        >
          <div className="mb-4">
            <h2 className="text-xl font-semibold">
              Đơn hàng {order?.code || `HD${order?.id}`} — {order?.createdAt ? dateFormat(order.createdAt) : ''}
            </h2>
          </div>
          ...
          <div className="space-y-4">
            <div className="rounded border p-3 text-sm">
              <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                <div>
                  ...
                  <div className="font-medium">{order?.code || `HD${order?.id}`}</div>
                </div>
                <div>
                  ...
                  <div className="font-medium">
                    ...
                    {customer.phone_number ? (
                      <span className="ml-1 text-xs text-muted-foreground">({customer.phone_number})</span>
                    ) : null}
                  </div>
                </div>
                <div>
                  <div className="text-muted-foreground">Ngày tạo</div>
                  <div className="font-medium">{order?.createdAt ? dateFormat(order.createdAt) : ''}</div>
                </div>
              </div>
            </div>
                ...
                <tbody>
                  {items.map((it) => (
                    <tr key={it.id} className="border-t">
                      ...
                      <td className="px-3 py-2">{formatCurrency(it.salePrice)}</td>
                      <td className="px-3 py-2">{formatCurrency(it.totalPrice)}</td>
                    </tr>
                  ))}
                  {items.length === 0 && (
                    <tr>
                      <td className="px-3 py-2 text-center italic text-muted-foreground" colSpan={5}>
                        Không có sản phẩm
                      </td>
                    </tr>
                  )}
            ...
            <div className="flex justify-end gap-8 text-sm">
              <div className="font-medium">
                Tổng tiền: <span className="font-bold">{formatCurrency(order?.totalAmount)}</span>
              </div>
            </div>
          </div>
...
export default InvoiceSimpleDialog
